...
An NLP comparison project for classifying political interview answers as **Clear Reply**, **Ambivalent**, or **Clear Non-Reply**, fine-tuning two transformer encoders and comparing efficiency against capacity.

This is the transformer half of a two-stage comparison; the classical baseline (TF-IDF, Word2Vec, logistic regression) that these results are measured against lives in [`political-evasion-classical-baseline`](https://github.com/panteliskts/political-evasion-classical-baseline).
...
1. **DistilBERT** — sentence-pair tokenization, weighted cross-entropy, AdamW, linear warm-up/decay, gradient clipping, stratified validation, and early stopping.
2. **DeBERTa-v3** — dynamic padding, mixed precision, label smoothing, class weighting, checkpoint selection, per-class evaluation, and length-based subgroup analysis.
...
Run the notebooks in numerical order. Both benefit from a CUDA GPU; reduce batch size if memory is limited.
...
Model weights are downloaded by their respective libraries and are never stored in this repository.
...
- Confusion matrices, training curves, and answer-length subgroup analysis
...
Raw interview text, processed dataset copies, model checkpoints, submissions, and caches are intentionally excluded.
...
Pantelis Kotsas's original code and documentation in this repository are MIT licensed. The dataset, pretrained models, source interview text, and generated model artifacts remain governed by their respective upstream terms; see [LICENSE](LICENSE).
